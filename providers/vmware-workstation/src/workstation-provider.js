...
export class WorkstationProvider{
  ...
  async testConnection(){await this.#request('/api/vms');return {status:'healthy',provider:'vmware_workstation',api:'VMware Workstation Pro REST API',capabilities:['inventory','power.start','power.stop','power.restart','power.pause']};}
  ...
  async execute(operation,nativeId){if(!['start','stop','restart','pause'].includes(operation))throw new WorkstationProviderError('NUV_OPERATION_UNSUPPORTED',`Unsupported VMware Workstation operation: ${operation}`);if(operation==='restart')await this.#powerRequest(nativeId,'off');await this.#powerRequest(nativeId,operation==='stop'?'off':operation==='pause'?'pause':'on');const reference=`workstation:${randomUUID()}`;this.#references.set(reference,{nativeId,operation});return {providerReference:reference,operation,targetId:nativeId};}
  async verify(reference){const operation=this.#references.get(reference);if(!operation)throw new WorkstationProviderError('NUV_WORKSTATION_REFERENCE_INVALID','Provider operation reference is unknown.');try{const power=await this.#request(`/api/vms/${encodeURIComponent(operation.nativeId)}/power`),state=this.#power(power.power_state??power.state),expected=operation.operation==='stop'?'stopped':operation.operation==='pause'?'suspended':'running';if(state!==expected)throw new WorkstationProviderError('NUV_WORKSTATION_VERIFICATION_PENDING',`Expected ${expected}, observed ${state}.`,{retryable:true});return {code:'NUV_OPERATION_VERIFIED',summary:'VMware Workstation reported the requested final VM state.',providerReference:reference,observedFinalState:state};}finally{this.#references.delete(reference);}}
