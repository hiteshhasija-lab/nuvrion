...
export class IdentityService {
  ...
  authenticate(username,password) {
    ...
    const session={ idHash:createHash('sha256').update(token).digest('hex'), userId:user.id, csrf, createdAt:Date.now(), lastSeenAt:Date.now(), idleExpiresAt:Date.now()+this.#idleTimeoutMs, absoluteExpiresAt:Date.now()+this.#absoluteTimeoutMs };
    this.#sessions.set(session.idHash,session); return { token, csrf, user:this.publicUser(user), expiresAt:new Date(session.idleExpiresAt).toISOString(),policy:{idleTimeoutMs:this.#idleTimeoutMs,absoluteTimeoutMs:this.#absoluteTimeoutMs} };
  }
  session(token) {
    ...
    session.lastSeenAt=now;session.idleExpiresAt=Math.min(now+this.#idleTimeoutMs,session.absoluteExpiresAt); const user=this.#users.get(session.userId); return user?{session,user:this.publicUser(user)}:null;
  }
