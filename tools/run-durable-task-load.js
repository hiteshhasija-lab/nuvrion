...
try{
  ...
  const pending=await store.pool.query("SELECT count(*)::int count FROM operations.outbox_messages o JOIN operations.tasks t ON t.task_id=o.message_key::uuid WHERE t.target_type='qualification_synthetic' AND t.target_id LIKE $1 AND o.published_at IS NULL",[`qualification:${evidence.runId}:%`]);
